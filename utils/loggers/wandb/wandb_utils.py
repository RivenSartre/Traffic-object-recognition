...
class WandbLogger():
    ...
    def __init__(self, opt, run_id=None, job_type='Training'):
       ...
       """
        ...
        #  but useful pre_config data is overwritten in the WandbLogger's wandb.init call
        if isinstance(opt.resume, str):  # checks resume from artifact
            if opt.resume.startswith(WANDB_ARTIFACT_PREFIX):
                entity, project, run_id, model_artifact_name = get_run_info(opt.resume)
                model_artifact_name = WANDB_ARTIFACT_PREFIX + model_artifact_name
                assert wandb, 'install wandb to resume wandb runs'
                # Resume wandb-artifact:// runs here| workaround for not overwriting wandb.pre_config
        ...
        if self.wandb_run:
            if self.job_type == 'Training':
                ...
                if isinstance(opt.data, dict):
                    ...
                    # write data_dict to pre_config. useful for resuming from artifacts. Do this only when not resuming.
